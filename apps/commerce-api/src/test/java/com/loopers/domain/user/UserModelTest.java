...
public class UserModelTest {
  ...
  class JoinTest {
    ...
    void throwIllegalArgumentException_whenUserIdIsAlphaNumericAndLengthAtLeast10(String userId) {
      // arrange

      // act
      Exception result = assertThrows(IllegalArgumentException.class, () -> new UserId(userId));
      // assert
    }

    @DisplayName("이메일 형식`aaa@yyy.com`이 틀리면, `IllegalArgumentException` 예외가 발생합니다.")
    @ParameterizedTest
    @ValueSource(strings = {"email", // ID만 존재하는 경우
        "email@" // @까지 존재하는 경우"
    })
    void throwsIllegalArgumentException_whenEmailIsWrongPattern(String email) {
      // arrange
      // act
      Exception result = assertThrows(IllegalArgumentException.class, () -> new Email(email));
      // assert
    }

    @DisplayName("생년월일형식`yyyy-MM-dd` 이 틀리면, `IllegalArgumentException` 예외가 발생합니다.")
    @Test
    void throwsBadRequestException_whenBirthIsWrongPattern() {
      // arrange
      String birth = "wrong";
      // act
      Exception result = assertThrows(IllegalArgumentException.class, () -> new BirthDay(birth));
